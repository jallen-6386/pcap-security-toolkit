...
- **Parallel stream export** — when `--export-streams` is set, the per-stream follow passes are run concurrently. Only the raw follow is fetched per stream and the ascii view is derived from it, halving the per-stream TShark passes (and yielding cleaner content, since the ascii-follow framing markers no longer leak into extracted payloads).
...
- **Streaming extraction (bounded memory)** — TShark output is parsed incrementally rather than buffered whole, and the two per-packet passes are reduced as they stream: the TCP stream index keeps only the first packet per stream, and the stream-triage stats are folded into per-stream aggregates on the fly. The Scapy stage likewise keeps only **online inter-arrival statistics per flow** for beaconing (count, Σδ, Σδ², last, min) rather than a timestamp per packet. Memory for these scales with the number of streams/flows, not the packet count, so multi-million-packet captures don't materialize a row per packet.
